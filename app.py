...
def get_clue_by_date(requested_date):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT date, clue, answer, hint, answer_length FROM clues WHERE date = ?", (requested_date,))
    row = cursor.fetchone()
    conn.close()
    if row:
        return {"date": row[0], "clue": row[1], "answer": row[2], "hint": row[3], "answer_length": row[4]}
    return None
